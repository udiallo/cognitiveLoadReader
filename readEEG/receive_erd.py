...
class Lsl_receiver:
    ...
    def __init__(self):
        ...
        self.data_buffer = deque(
            maxlen=125 * 5)  # efficient implemented list object, thread safe, 256*5 == 5 seconds (with sampling rate 256)
    ...
    def grab_data(self):
        while self.recording:
            data, times = self.inlet.pull_chunk(
                timeout=0.0)  # data contains sample (e.g. 16 values for 16 channels), if CPU to slow, data can contain more than one sample (list of lists == chunks)
    ...
    def cut_segment(self, timestamp,
                    segment_size_in_sec):  # timestamp we are interested in, second argument: segment_size
        data_segment = None
        time_segment = None
        with self.lock:
            ...
            # print("Time Segment", time_segment)
    ...
    # Calculates Cognitive Load adapted from Horn (2009)
    def cl_index(self, segment_powers, alpha_channel, theta_channel):
        ...
        #print("segment powerrs aus cl index",segment_powers)
...
if __name__ == '__main__':
    ...
    alpha_channel = [0]  # [15]  # choose channel of interest for alpha_band
    theta_channel = [1]  # [11]  # choose channel of interest for theta_band
    ...
    try:  # calculate threshold
        ...
        try:
            while True:
                # not lsl.threshold_calculated:
                time.sleep(1)  # time.sleep, otherwise CPU will be 100% used

                data, times = lsl.cut_segment(local_clock() - 5.5, 5)

                # calculates baseline before test // mean von 34 sek in counter = 34
                if counter < 34:
                    baseline_power = band.calculate_abspower(data, times, 125)    # baseline == baseline_band_mat from bandpower.py //  [theta, alpha]
                ...
                if counter == 34:
                    ...
                    print("Baseline Mean Value of Theta Channel and Alpha Channel  == ",theta_channel, alpha_channel, baseline)
                #    cli_list = []
                #    lsl.start_task2()
                ...
                segment_powers = band.calculate_bandpower(data, times, 125)  # gibt rel. Bänder raus

                band_mat.append(segment_powers)  # band_mat == band powers

                # gibt Theta/Alpha raus
    ...
    try:
        ...
        try:
            while True:
                ...
                #############################################################################
                # ERD/ERS
                #cog_load = lsl.erd_ers(absolute_power, alpha_channel, theta_channel)

                cli_list.append(cli)

                #############################################################################
                cog_load_list.append(cog_load)
